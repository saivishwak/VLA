...
class PackingShapes(Task):
    ...
    def reset(self, env):
        ...
        n_objects = 5
        if self.mode == 'train':
            obj_shapes = np.random.choice(
                self.train_set, n_objects, replace=False)
        else:
            if self.homogeneous:
                obj_shapes = [np.random.choice(
                    self.test_set, replace=False)] * n_objects
            else:
                obj_shapes = np.random.choice(
                    self.test_set, n_objects, replace=False)
        ...
        for i in range(num_objects_to_pick):
            ...
            self.lang_goals.append(
                self.lang_template.format(obj=shapes[obj_shapes[i]]))
