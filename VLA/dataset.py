    ...
    def __init__(self, path, cfg, n_demos=0, augment=False):
        ...
        if self.n_demos > 0:
            ...
            if self.n_demos > self.n_episodes:
                raise Exception(f"Requested training on {self.n_demos} demos, but only {
                                self.n_episodes} demos exist in the dataset path: {self._path}.")

            episodes = np.random.choice(
                range(self.n_episodes), self.n_demos, False)
            self.set(episodes)
...
class RavensMultiTaskDataset(RavensDataset):
    ...
    def __init__(self, path, cfg, group='multi-all',
        ...
        self.attr_train_task = self.MULTI_TASKS[group][
            'attr_train_task'] if 'attr_train_task' in self.MULTI_TASKS[group] else None
        ...
        if self.n_demos > 0:
            self.images = self.cfg['dataset']['images']
            # TODO(mohit): fix caching for multi-task dataset
            self.cache = False
            self.set(episodes)
    ...
    def load(self, episode_id, images=True, cache=False):
        if self.attr_train_task is None or self.mode in ['val', 'test']:
            ...
            sampling_probs = [(1-mult_attr_seen_sample_prob) /
                              (len(all_tasks)-1)] * len(all_tasks)
