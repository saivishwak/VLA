...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        assert os.path.exists(assets_root), f"Assets root {
            assets_root} does not exist!"
    ...
    def add_video_frame(self):
        ...
        color, depth, _ = self.render_camera(
            config, image_size=image_size, shadow=0)
    ...
    def render_camera(self, config, image_size=None, shadow=1):
        ...
        # image_size is explicity set in recording mode else its already present in agent cams
        if not image_size:
        ...
        segm = np.array(segm).reshape(depth_image_size)
