...
import numpy as np
import hydra
from VLA import agents
from VLA import dataset
...
def main(vcfg):
    ...
    json_name = f"multi-results-{
        mode}.json" if 'multi' in vcfg['model_path'] else f"results-{mode}.json"
...
def list_ckpts_to_eval(vcfg, existing_results):
    ...
    # Validation checkpoints that haven't been already evaluated.
        ...
        checkpoints = sorted([c for c in os.listdir(
            vcfg['model_path']) if "steps=" in c])
    ...
    # Find the best checkpoint from validation and run eval on the test set.
        ...
        result_jsons = [c for c in os.listdir(
            vcfg['results_path']) if "results-val" in c]
        if 'multi' in vcfg['model_task']:
    ...
    # Load a specific checkpoint with a substring e.g: 'steps=10000'
        ...
        checkpoints = [c for c in os.listdir(
            vcfg['model_path']) if vcfg['checkpoint_type'] in c]
